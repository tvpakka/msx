...
// Add an execute button to force immediate search
...
function SearchHandler() {
    ...
    var SEARCH_DELAY = 10000; // 10 seconds delay
    ...
    var updateSearch = function() {
        ...
        if (resultItems && resultItems.length > 0) {
            createPages(resultItems, function(resultItem, pageItem, index) {
                // Additional processing if needed
            }, function(page) {
                search.pages.push(page);
            });
            inputPage.headline = RESULTS_HEADLINE.replace("{ITEMS}", resultItems.length + " " + (resultItems.length == 1 ? "Movie" : "Movies"));
        } else {
